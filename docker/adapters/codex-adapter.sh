...
set -eu
workspace=$1 prompt_file=$2 bench_home=$3 wall=$4 result=$5 final=$6 model=${7} reasoning=${8}
...
HOME="$bench_home" timeout --foreground "$wall" codex exec --json --ephemeral --skip-git-repo-check \
  --ignore-user-config --ignore-rules --strict-config --model "$model" \
...
python3 /opt/bench/normalize.py codex "$raw" "$bench_home/.dsh/sessions" "$result" "$final" "$code" "$(awk "BEGIN {print $end-$start}")" "$raw_stderr" "$model" "$reasoning"
exit 0
